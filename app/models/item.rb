...
class Item < ActiveRecord::Base
  ...
  attr_accessible :property_type, :description, :image, :image_cache, :remove_image, :name, :rent, :available_from, :number_of_bathrooms, :number_of_rooms, :number_of_study, :lease_duration, :postal_code, :unit_number, :furnished
 ...
 # validates_date :available_from, :on_or_after => Date.today, :on_or_after_message => "date must be on or after today"
  ...
  def sanitize_item
    ...
    self.property_type = Sanitize.clean(self.property_type).gsub('&amp;','&') unless self.property_type.blank?
